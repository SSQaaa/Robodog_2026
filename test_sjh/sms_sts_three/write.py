...
while True:
    ...
    if index == 1:
        ...
        # 下面的程序是防止舵机堵转烧坏的程序
        time.sleep(2)   # 这个延时很重要，要确保舵机转到指定位置才进行下面的舵机状态读取程序

        # 读取舵机当前状态下的电流和位置，以电流为依据判断舵机是否堵转发热，尽可能避免堵转烧毁舵机
        # 电流越大，舵机堵转烧毁的可能就越大。要适当调整传入的角度。由于安装结构的原因，角度调整可能变大或变小才能使舵机位置回退减少舵机烧毁风险。
        # 读取电流和当前位置
        SCS_ID_1_I, SCS_ID_1_position = packetHandler.ReadPosStatus(SCS_ID_1)
        print("当前的电流和位置:",SCS_ID_1_I, SCS_ID_1_position)
        # 以电流为依据判断舵机是否堵转，并调整
        while True:
            # 获取当前的电流和位置
            SCS_ID_1_I, SCS_ID_1_position = packetHandler.ReadPosStatus(SCS_ID_1)
            print("[ID:%03d] I:%2d PresPos:%d" % (SCS_ID_1, SCS_ID_1_I, SCS_ID_1_position))
            packetHandler.WritePosEx(SCS_ID_1, SCS_ID_1_position, SCS_MOVING_SPEED, SCS_MOVING_ACC)

            time.sleep(0.05)
            if SCS_ID_1_I > 30:
                print("抓取力量太大")
                SCS_ID_1_position = SCS_ID_1_position - 1   # 将当前的数值减去一定的值
                print("当前值:",SCS_ID_1_position)
                print("力量调整！")
