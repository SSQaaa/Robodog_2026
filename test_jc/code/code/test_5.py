...
class DetectImageTRT:
    def __init__(self, vis_dir="vis_results"):
        ...
        print(f"可视化结果将保存至: {os.path.abspath(self.vis_dir)}")
        # 预热引擎（解决首次推理耗时偏高问题）
        self.warmup_engine()
    ...
    def infer_single(self, frame, img_name, save_vis=True):
        ...
        # 后处理筛选检测框
        vis_img, pred_boxes, pred_cls_ids = self.draw_boxes(frame.copy(), result_boxes, result_scores, result_classid)
        
        
        # 保存可视化结果
        if save_vis:
            save_path = os.path.join(self.vis_dir, img_name)
    ...
    def run_evaluation(self):
        ...
        for img_path in tqdm(img_files, total=len(img_files)):
            ...
            if frame is None:
                ...
                continue
            # 单张推理
            self.infer_single(frame, img_name, save_vis=True)
        print(f"\n可视化结果已保存至: {os.path.abspath(self.vis_dir)} ({len(img_files)}张图片)")
