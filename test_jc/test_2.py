...
CONF_THRESH = 0.25
# ====================== 修复1：提高置信度，减少误检/重复框 ======================
CONF_THRESH_DASHBOARD = 0.35  # 从0.1提高到0.35
IOU_THRESHOLD = 0.45          # 稍微收紧NMS
DETECTION_SIZE = 38
DASHBOARD_ID = 6
SSI_ID = 7
...
class YoLov5TRT:
    ...
    def post_process(self, output, origin_h, origin_w):
        ...
        valid_class = (classid >= 0) & (classid < len(CLASS_NAMES))
...
def _state_from_dashboard_old(image_raw, dashboard_box, ssi_box):
    ...
    # 基准向量：表盘中心 → SSI（固定0°参考）
    v_ref_x = ssi_center[0] - dashboard_center[0]
    v_ref_y = ssi_center[1] - dashboard_center[1]

    # 指针向量：表盘中心 → 指针
    v_ptr_x = pointer_point[0] - dashboard_center[0]
    v_ptr_y = pointer_point[1] - dashboard_center[1]

    # ====================== 核心修复：相对角度 ======================
    # 计算 有符号夹角（-180 ~ +180），仪表盘怎么斜都不影响
    angle_ref = math.atan2(v_ref_y, v_ref_x)
    angle_ptr = math.atan2(v_ptr_y, v_ptr_x)
    delta_angle = math.degrees(angle_ptr - angle_ref)

    # 转到 0~360 区间
    delta_angle = delta_angle % 360
    # =================================================================

    # 【相对区间】：不管表盘怎么装，这三个区间永远正确
    NORMAL_LOW = 130
    NORMAL_HIGH = 230

    if NORMAL_LOW <= delta_angle <= NORMAL_HIGH:
        return "正常"
    elif delta_angle < NORMAL_LOW:
        return "偏低"
    else:
        return "偏高"
...
def print_dashboard_result(analyze_result, tag):
    ...
    for state_item in analyze_result['state_list']:
        n, state = state_item
        result_str += f"  表计{n}：{state}\n"
    # 只有结果变化时才打印
...
class SimpleInfer:
    ...
    def _show_infer_frame(self, infer_output, analyze_result, use_time):
        ...
        for state_item in analyze_result['state_list']:
            n, state = state_item
            xyxy = analyze_result['xyxy_list'][n-1]
            _, x1, y1, _, _ = xyxy
