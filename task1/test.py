...
# -*- coding: utf-8 -*-
"""
4m × 1.7m 锥桶避障穿越任务（平移绕过版）
前提：已用手柄让机器狗站立，并处于可移动状态。
逻辑：直行 → 遇到锥桶 → 横向平移绕过 → 继续直行 → 累计前进 4 米后停止。
"""
...
TRT_LIB_DIR = os.path.join(BASE_DIR, "libs")
...
VY_SHIFT = 30000            # 平移速度
SHIFT_DURATION = 0.6        # 平移持续时间（秒）

# --- 真实速度标定（必填：你的实测速度）---
REAL_SPEED_VX = 0.5         # 米/秒，请替换为实测值
...
def compute_avoidance_action(x, z,
    ...
    if z > safe_dist:
        ...
        if abs(x) < lat_thresh:
            if x >= 0:
                return 0, -VY_SHIFT, 0, "shift_right"
            else:
        ...
        while forward_distance < TARGET_DISTANCE:
            ...
            if shift_active:
                ...
                if cones_3d:
                    ...
                    print(f"[Avoid] cone x={x:.3f} z={z:.3f} -> {action} vx={vx} vy={vy} vz={vz}")
                else:
