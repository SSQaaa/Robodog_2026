...
def pixel_to_camera(pixel_x, pixel_y, depth_mm, intrinsics):
    fx = float(intrinsics["fx"])
    fy = float(intrinsics["fy"])
    cx = float(intrinsics["cx"])
    cy = float(intrinsics["cy"])

    x_mm = (float(pixel_x) - cx) * float(depth_mm) / fx
    y_mm = (float(pixel_y) - cy) * float(depth_mm) / fy
    z_mm = float(depth_mm)
...
def main():
    ...
    color_intrinsics = cam.get_color_intrinsics()
    depth_intrinsics = cam.get_depth_intrinsics()

    print(f"[Orbbec] color size: {color_w}x{color_h}")
    print(f"[Orbbec] depth size : {depth_w}x{depth_h}")
    print(f"[Orbbec] rotate_180: {cam.is_rotate_180_enabled()}")
